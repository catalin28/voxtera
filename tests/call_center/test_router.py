...
def test_hybrid_with_hotel_mention_routes_to_hybrid() -> None:
    out = SourceRouter().route(_decomp(
        query_type="hybrid", intent="local_operator", hotel_mention="Rixos Belek"
    ))
    assert out["path"] == PATH_HYBRID
...
def test_scoped_with_resolved_hotel_routes_to_scoped() -> None:
    out = SourceRouter().route(
        _decomp(query_type="scoped", intent="amenities", hotel_mention="Rixos Belek"),
        session={"active_hotel_id": "rixos_belek"},
    )
...
def test_scoped_with_unresolved_hotel_routes_to_resolver() -> None:
    out = SourceRouter().route(_decomp(
        query_type="scoped", intent="amenities", hotel_mention="Rixosta"
    ))
...
def test_hotel_mention_without_scoped_query_type_still_resolves_hotel() -> None:
    ...
    out = SourceRouter().route(_decomp(
        query_type="broad", intent="amenities", hotel_mention="Rixos", region="antalya"
    ))
    assert out["path"] == PATH_HOTEL_RESOLVE
...
def test_destination_query_type_routes_to_destination_kb() -> None:
    out = SourceRouter().route(_decomp(
        query_type="destination", intent="destination_info", city="Cappadocia"
    ))
