...
PSTN_WEBHOOK_URL="${PSTN_WEBHOOK_URL:-https://64.225.99.72.sslip.io/pstn/webhook}"
# Frankfurt has no PSTN. Default OFF so this box never reassigns the Daily
# pinless dial-in webhook (account-global) away from Toronto.
CONFIGURE_PINLESS_DIALIN="${CONFIGURE_PINLESS_DIALIN:-false}"

# Local env file shipped to the server. Frankfurt uses .env.fra (EU settings:
# GLADIA_REGION=eu-west, actions/PSTN off), NOT Toronto's .env.
LOCAL_ENV_FILE="${LOCAL_ENV_FILE:-.env.fra}"

# Builds this droplet's LOCAL hotel RAG — `voxtera ingest` embeds the hotel
# files into a per-droplet SQLite chunks store (voxtera.rag.store.ChunksStore)
# that the demo bot reads locally. Frankfurt must build its own, so keep ON.
...
usage() {
  cat <<'EOF'
Usage: scripts/deploy-fra.sh [options]

Deploy Voxtera to the Frankfurt (EU) droplet. Ships .env.fra and leaves
PSTN dial-in and RAG ingest OFF by default.
...
Examples:
  scripts/deploy-fra.sh
  scripts/deploy-fra.sh --host voxtera-fra
  scripts/deploy-fra.sh --skip-sync
EOF
}
...
if [[ -f "${LOCAL_ENV_FILE}" ]]; then
  echo "==> Deploying ${LOCAL_ENV_FILE} to ${HOST}:${REMOTE_ENV_FILE}"
  ...
  scp "${LOCAL_ENV_FILE}" "${HOST}:${REMOTE_ENV_FILE}"
  ...
  if ! grep -q '^PSTN_HMAC_VERIFY=' "${LOCAL_ENV_FILE}" && [[ -n "${REMOTE_PSTN_HMAC_VERIFY}" ]]; then
  ...
  echo "==> No local ${LOCAL_ENV_FILE} found, skipping env file deploy"
fi

if [[ "$CONFIGURE_PINLESS_DIALIN" == "true" ]]; then
  if [[ ! -f "${LOCAL_ENV_FILE}" ]]; then
    echo "Error: ${LOCAL_ENV_FILE} is required to configure Daily pinless dial-in." >&2
    exit 1
  fi
  ...
  if [[ -z "${DAILY_API_KEY:-}" || -z "${PSTN_PHONE_NUMBER:-}" || -z "${PSTN_WEBHOOK_HMAC:-}" ]]; then
    echo "Error: DAILY_API_KEY, PSTN_PHONE_NUMBER, and PSTN_WEBHOOK_HMAC must be set in ${LOCAL_ENV_FILE}" >&2
    exit 1
  fi
